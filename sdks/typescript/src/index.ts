...
export class FarmEase {
  ...
  private async request<T>(method:string,path:string,body?:unknown):Promise<T>{const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),this.timeoutMs);try{const response=await this.fetcher(`${this.options.baseUrl.replace(/\/$/,'')}${path}`,{method,headers:{'Accept':'application/json',...(body?{'Content-Type':'application/json'}:{}),...(this.options.token?{Authorization:`Bearer ${this.options.token}`}:{})},body:body?JSON.stringify(body):undefined,signal:controller.signal});const json=await response.json() as ApiEnvelope<T>&{error?:{message:string;code?:string;details?:unknown}};if(!response.ok||json.error)throw new FarmEaseError(json.error?.message||`FarmEase request failed (${response.status})`,response.status,json.error?.code,json.error?.details);return json.data;}catch(e){if(e instanceof FarmEaseError)throw e;throw new FarmEaseError(`FarmEase request failed: ${(e as Error).message}`);}finally{clearTimeout(timer);}}
